...
G1SrbdMpc::Input buildWalkingMpcInput(const DataBus &robotState,
                                      ...
                                      double mpcSegmentDt)
{
    ...
    input.reference.resize(13, kHorizon);
...
{
    std::cout << "\n"
              << "======== G1 SRBD-MPC + Walk-WBC Footstep IK Walk  t="
...
int main(int argc, const char **argv)
{
    ...
    G1SrbdMpc srbdMpc(kHorizon, mpcSegmentDt);
    ...
    if (!options.headless)
    {
        ...
        uiController.createWindow("G1 Hector-style SRBD-MPC + IK Walk", false);
    }
    ...
    while ((options.headless || !glfwWindowShouldClose(uiController.window)) && mj_data->time < options.duration)
    {
               ...
               mj_data->time < options.duration)
        {
            ...
            {
                const G1SrbdMpc::Input mpcInput =
            ...
            const Eigen::VectorXd tauFeedforward =
                contactTorqueFeedforward(robotState, lastMpcPlaneForces, ffRamp * options.ffScale);
