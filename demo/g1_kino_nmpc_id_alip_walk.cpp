...
constexpr double kDefaultSingleSupportTime = 0.35;
constexpr double kDefaultDoubleSupportTime = 0.04;
...
constexpr double kDemoStepXMax = 0.06;
constexpr double kDemoStepWidthMin = 0.20;
constexpr double kDemoStepWidthMax = 0.25;
...
Eigen::Matrix<int, Eigen::Dynamic, G1KinodynamicsNmpc::kNumFeet>
constantContactTable(bool leftContact, bool rightContact, int horizon)
{
    Eigen::Matrix<int, Eigen::Dynamic, G1KinodynamicsNmpc::kNumFeet> table(
        std::max(1, horizon), G1KinodynamicsNmpc::kNumFeet);
    table.col(0).setConstant(leftContact ? 1 : 0);
    table.col(1).setConstant(rightContact ? 1 : 0);
    return table;
}
...
void clampDemoSwingFootPlan(DataBus &robotState)
{
    ...
    current.z() += std::max(0.0, oldCurrent.z() - oldLine.z());
    ...
    while ((options.headless || !glfwWindowShouldClose(uiController.window)) && mj_data->time < options.duration)
    {
               ...
               mj_data->time < options.duration)
        {
            ...
            const Eigen::Matrix<int, Eigen::Dynamic, G1KinodynamicsNmpc::kNumFeet> contactTable =
                constantContactTable(leftContact, rightContact, options.horizon);
