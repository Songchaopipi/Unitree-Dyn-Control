...
G1SrbdMpcRpEnhanced::Input buildWalkingMpcInput(const DataBus &robotState,
                                      ...
                                      double mpcSegmentDt)
{
    G1SrbdMpcRpEnhanced::Input input;
    ...
    input.reference.resize(G1SrbdMpcRpEnhanced::kNx, kHorizon);
...
{
    std::cout << "\n"
              << "======== G1 Enhanced Torso-RP SRBD-MPC + IK Walk  t="
...
int main(int argc, const char **argv)
{
    ...
    const std::string logPath =
        firstExistingPath("../record/matlabReadDataScript.txt", "record/matlabReadDataScript.txt") ==
                "../record/matlabReadDataScript.txt" ?
            "../record/g1_srbd_ik_walk_rp_enhanced.log" :
            "record/g1_srbd_ik_walk_rp_enhanced.log";
    const std::string lowerBodyCsvPath = outputPathNearRecordScript("g1_srbd_ik_walk_rp_enhanced_lower_body.csv");
    ...
    G1SrbdMpcRpEnhanced srbdMpc(kHorizon, mpcSegmentDt);
    ...
    if (!options.headless)
    {
        ...
        uiController.createWindow("G1 Enhanced Torso-RP SRBD-MPC + IK Walk", options.recordVideo, videoRawPath.c_str());
    }
    ...
    while ((options.headless || !glfwWindowShouldClose(uiController.window)) && mj_data->time < options.duration)
    {
               ...
               mj_data->time < options.duration)
        {
            ...
            {
                const G1SrbdMpcRpEnhanced::Input mpcInput =
