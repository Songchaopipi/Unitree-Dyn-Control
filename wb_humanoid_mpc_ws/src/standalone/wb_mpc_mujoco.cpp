...
namespace demo = wb_humanoid::wb_mpc_mujoco;
...
int main(int argc, const char** argv) {
  ...
  try {
    ...
    ocs2::humanoid::WBMpcInterface interface(paths.taskFile, paths.urdfFile, paths.referenceFile);
    ocs2::SqpMpc mpc(interface.mpcSettings(), interface.sqpSettings(), interface.getOptimalControlProblem(), interface.getInitializer());

    ocs2::humanoid::WBMpcTargetTrajectoriesCalculator targetCalculator(
        paths.referenceFile,
    ...
    if (options.threadedMpc) {
      std::cout << "[wb_mpc_mujoco] controller: threaded WBMpcMrtJointController" << std::endl;
      ocs2::humanoid::WBMpcMrtJointController jointController(interface.modelSettings(),
                                                              mpc,
                                                              interface.getPinocchioInterface(),
                                                              interface.mpcSettings().mpcDesiredFrequency_);

      jointController.startMpcThread(demo::makeWbRobotState(robotState, interface.modelSettings()));
      while (!jointController.ready()) {
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
      }

      if (options.precompileOnly) {
        std::cout << "[wb_mpc_mujoco] threaded precompile/load-only path completed. base_z="
                  << robotState.basePosition.z() << std::endl;
        return 0;
      }

      std::vector<ocs2::humanoid::WBMpcJointAction> jointActions = demo::makeHoldActions(standPose);
      ...
      while (simInterface.time() <= options.simEndTime && (viewer == nullptr || !viewer->shouldClose())) {
        ...
        if (robotState.time + 0.5 * demo::kSimDt >= nextMrtTime) {
          jointController.computeJointControlAction(demo::makeWbRobotState(robotState, interface.modelSettings()), jointActions);
        ...
        if (demo::stateLooksInvalid(robotState, appliedTorques, invalidReason)) {
          std::cerr << "[wb_mpc_mujoco] validation failed at t=" << robotState.time << ": " << invalidReason << std::endl;
    ...
    std::cout << "[wb_mpc_mujoco] controller: single-thread MPC_MRT_Interface" << std::endl;
    ...
    std::cout << "Running first whole-body NMPC solve..." << std::endl;
    ...
    for (int i = 0; i < options.warmupSolves; ++i) {
      ...
      if (!mrt.updatePolicy()) {
        throw std::runtime_error("WB MPC warmup failed to publish a policy");
      }
    }

    if (options.precompileOnly) {
      std::cout << "[wb_mpc_mujoco] precompile/load-only path completed. base_z="
                << robotState.basePosition.z() << std::endl;
      return 0;
    }
    ...
    while (simInterface.time() <= options.simEndTime && (viewer == nullptr || !viewer->shouldClose())) {
      ...
      if (mpcUpdateDt > 0.0 && robotState.time + 0.5 * demo::kSimDt >= nextMpcTime) {
        ...
        if (!mrt.updatePolicy()) {
          std::cerr << "[wb_mpc_mujoco] MPC did not publish a new policy at t=" << robotState.time << std::endl;
        }
    ...
    std::cerr << "wb_mpc_mujoco failed: " << e.what() << std::endl;
    return 1;
  }
